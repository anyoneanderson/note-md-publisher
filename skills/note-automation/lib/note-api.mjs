/**
 * note.com API Client - 2-step draft posting
 *
 * Step 1: POST /api/v1/text_notes  -> create article, get { id, key }
 * Step 2: POST /api/v1/text_notes/draft_save?id={articleId} -> update body
 *
 * Note: note.com has no API endpoint for publishing.
 *       Articles are always saved as drafts. Publish manually via the web UI.
 *
 * @module note-api
 */
 ...
 *
 * draft_save always saves as draft. note.com has no publish API.
 *
 * @param {object} params
...
export async function updateArticle({
  ...
  const headers = buildHeaders(cookies);
  const payload = { body: htmlContent, name: title };
  if (imageUrl) {
...
/**
 * Post an article to note.com as draft using the 2-step flow.
 *
 * Step 1 creates the article and returns id + key.
 * Step 2 updates the article body and optional header image.
 *
 * @param {object} params
 * @param {string} params.htmlContent - Converted HTML body
 * @param {string} params.title - Article title
 ...
 * @returns {Promise<{success: boolean, articleId: number, articleKey: string, noteUrl: string}>}
 */
